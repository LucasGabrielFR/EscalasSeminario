...
const Sidebar: React.FC<SidebarProps> = ({ view, setView }) => {
  ...
  return (
    <nav className="w-full md:w-20 lg:w-64 bg-white border-r border-slate-200 p-4 flex md:flex-col gap-2 sticky top-0 z-50 shadow-sm print:hidden">
      ...
      <div className="hidden md:flex items-center justify-center lg:justify-start px-2 pt-4 mt-2 border-t border-slate-100">
        <span className="text-xs text-slate-400 font-medium tracking-wide hidden lg:block">
          v{__APP_VERSION__}
        </span>
        <span className="text-xs text-slate-400 font-medium tracking-wide lg:hidden" title={`v${__APP_VERSION__}`}>
          v
        </span>
      </div>
    </nav>
